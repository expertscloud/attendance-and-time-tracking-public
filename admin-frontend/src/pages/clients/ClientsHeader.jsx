...
import { resetFiltersAction, setSearchedText } from '@/store/reducers/clientsSlice';

const ClientsHeader = ({ refetch, isFetching }) => {
    ...
    <>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between', my: 1.5, gap: 1 }}>
        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 1 }}>
          ...
          <Tooltip title={isFetching ? 'Getting Fresh Clients...' : 'Refresh Clients'} arrow>
      ...
      {isCreateDialog ? <ClientForm onClose={() => setIsCreateDialog(false)} /> : null}
    </>
  );
};
